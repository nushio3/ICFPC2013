...
-- sEvalPure :: Map.Map Idfr SBitVector -> Expr -> SBitVector
-- sEvalPure m (If0 a b c) = ite (sEval m a .== 0) (sEval m b) (sEval m c)
-- sEvalPure m (Fold a b (Reducer v w e)) = 
--   foldr go (sEval m b) [shiftR va (i * 8) .&. 0xFF | i <- [0..7]]
--   where 
--     va = sEval m a
--     go x y = 
--       sEvalPure (m & at v ?~ x & at w ?~ y) e
--   
-- -- sEval m (Op1 Not e) = sOp1 complement m e
-- -- sEval m (Op1 Shl1 e) = sOp1 (flip shiftL 1) m e
-- -- sEval m (Op1 Shr1 e) = sOp1 (flip shiftR 1) m e
